...
let OverwatchProvider = function () {
    ...
    let parseTiers = (img) => {    
        ...
        return { tier: TIERS[m[1]], tierEmblem: img.split('background-image:url')[1] };
    }

    let parseStarRank = (img) => {
        if(!img)
            return '';

        return img.split('background-image:url')[1];
    }
    ...
    let parseProfile = ($) => {
        ...
        const tierData = parseTiers($('.player-level').attr('style').replace(/^url|[\(\)]/g, ''));
        stats.tier = tierData.tier;
        stats.tierEmblem = tierData.tierEmblem;
        stats.starRank = parseStarRank($('.player-rank').attr('style').replace(/^url|[\(\)]/g, ''));
        if (stats.rank)
        {
            stats.rankPicture = $('div.competitive-rank > img').attr('src');
            // season are not in picture url
    ...
    self.get = (platform, region, tag, overallOnly, lang = 'en-us') => {
        ...
        return rp(baseurl).then((context) => {
            ...
            var p = new Promise((resolve, reject) => {
                ...
                result.profile.bnet = `${result.profile.nick}#${tag.split('-')[1]}`;
